...
class JpaCrudTest {
    ...
    fun setup() {
        client = RestClient.builder()
            .baseUrl("http://localhost:$port/api/jpa")
            .build()
    }

    private fun uniqueEmail(): String = "jpa-${++emailCounter}-${System.nanoTime()}@test.com"
    ...
    fun `POST creates author with books`() {
        val request = CreateAuthorRequest(
            ...
            email = uniqueEmail(),
            books = listOf(
                BookDto(title = "Spring in Action", isbn = "J${System.nanoTime()}", price = BigDecimal("49.99")),
            ),
        )
        ...
        response.body?.name shouldBeEqualTo "JPA Author"
    ...
    fun `GET returns author by id`() {
        ...
        response.body?.name shouldBeEqualTo "Find JPA Author"
    }

    @Test
    fun `GET returns all authors`() {
        val request = CreateAuthorRequest(name = "List JPA Author", email = uniqueEmail())
        client.post()
            .uri("/authors")
            .contentType(MediaType.APPLICATION_JSON)
            .body(request)
    ...
    fun `PUT updates author`() {
        val request = CreateAuthorRequest(name = "JPA Original", email = uniqueEmail())
        val created = client.post()
            .uri("/authors")
            .contentType(MediaType.APPLICATION_JSON)
            .body(request)
        ...
        val updateRequest = CreateAuthorRequest(name = "JPA Updated", email = created.email)
    ...
    fun `DELETE removes author`() {
        val request = CreateAuthorRequest(name = "JPA To Delete", email = uniqueEmail())
        val created = client.post()
            .uri("/authors")
            .contentType(MediaType.APPLICATION_JSON)
            .body(request)
    ...
    fun `POST bulk creates multiple authors`() {
        val requests = listOf(
            CreateAuthorRequest(name = "JPA Bulk 1", email = uniqueEmail()),
            CreateAuthorRequest(name = "JPA Bulk 2", email = uniqueEmail()),
        )
        val response = client.post()
            .uri("/authors/bulk")
            .contentType(MediaType.APPLICATION_JSON)
