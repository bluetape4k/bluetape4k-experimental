...
import io.bluetape4k.examples.benchmark.service.JpaAuthorService
...
@RestController
@RequestMapping("/api/jpa/authors")
class JpaController(
    private val authorService: JpaAuthorService,
) {
    ...
    fun create(@RequestBody request: CreateAuthorRequest): ResponseEntity<AuthorDto> {
        ...
        return ResponseEntity.created(URI.create("/api/jpa/authors/${created.id}")).body(created)
    }
