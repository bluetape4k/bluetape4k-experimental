...
class BenchmarkContainerReuseTest {
    ...
    @Test
    fun `reusable Redis container survives the launcher JVM`() {
        assumeTrue(
    ...
    @Test
    fun `non-reusable Redis container stops with the launcher JVM`() {
    ...
    private fun launchProbe(reuse: Boolean): String {
        val process = ProcessBuilder(
            ...
            "io.bluetape4k.benchmark.cache.BenchmarkContainerReuseTestKt",
            "probe-reusable-redis",
        ).apply {
...
fun main(args: Array<String>) {
    if (args.singleOrNull() == "probe-reusable-redis") {
        println("CONTAINER_ID=${BenchmarkRedisSupport.redis.containerId}")
    }
}
