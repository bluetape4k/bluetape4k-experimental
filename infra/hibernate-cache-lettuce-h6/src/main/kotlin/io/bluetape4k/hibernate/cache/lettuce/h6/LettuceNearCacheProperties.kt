...
data class LettuceNearCacheProperties(
    ...
    companion object {
        ...
        fun from(configValues: Map<String, Any>): LettuceNearCacheProperties {
            fun str(
                key: String,
                default: String,
            ): String = configValues["$PREFIX$key"]?.toString() ?: default
                    ...
                    .associate { (k, v) ->
                        ...
                        regionName to (
                            parseDuration(v.toString())
                                ?: throw IllegalArgumentException("Invalid duration for '$k': $v")
                        )
                    }
            ...
            return LettuceNearCacheProperties(
                ...
                localExpireAfterWrite =
                    duration("local.expire_after_write", Duration.ofMinutes(30))
    ...
    /**
     * [LettuceBinaryCodecs]를 사용해 [LettuceBinaryCodec]을 직접 생성.
     */
    fun createCodec(): LettuceBinaryCodec<Any> =
    ...
    private fun validatePositiveDuration(
        name: String,
        duration: Duration?,
    ) {
