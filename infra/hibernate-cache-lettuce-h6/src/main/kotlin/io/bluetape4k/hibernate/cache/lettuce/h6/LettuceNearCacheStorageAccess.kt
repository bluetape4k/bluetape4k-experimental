 ...
 *
 * [LettuceNearCache]를 래핑하여 Hibernate 6.6.x 2nd level cache 브릿지 역할을 한다.
...
class LettuceNearCacheStorageAccess(
    ...
    private fun cacheKey(key: Any): String =
        when (key) {
            is BasicCacheKeyImplementation -> {
                "${key.entityOrRoleName}#${canonicalString(key.id)}"
            }
            is CacheKeyImplementation -> {
                "${key.entityOrRoleName}#${canonicalString(key.id)}"
            }
            is NaturalIdCacheKey -> {
                buildString {
    ...
    private fun canonicalNaturalIdValues(values: Any?): String =
        when (values) {
    ...
    private fun canonicalString(value: Any?): String =
        when (value) {
            null -> "null"
    ...
    ): Any? = nearCache.get(cacheKey(key))

    override fun putIntoCache(
        key: Any,
        value: Any,
        session: SharedSessionContractImplementor,
    ) {
    ...
    override fun contains(key: Any): Boolean = nearCache.containsKey(cacheKey(key))
