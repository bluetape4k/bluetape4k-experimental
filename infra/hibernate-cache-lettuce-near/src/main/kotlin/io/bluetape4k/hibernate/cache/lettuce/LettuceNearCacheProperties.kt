...
data class LettuceNearCacheProperties(
    ...
    fun createCodec(): LettuceBinaryCodec<Any> = when (codec.lowercase()) {
        "jdk"       -> LettuceBinaryCodec(BinarySerializers.Jdk)
        "kryo"      -> LettuceBinaryCodec(BinarySerializers.Kryo)
        "fory"      -> LettuceBinaryCodec(BinarySerializers.Fory)
        "lz4jdk"    -> LettuceBinaryCodec(BinarySerializers.LZ4Jdk)
        "lz4kryo"   -> LettuceBinaryCodec(BinarySerializers.LZ4Kryo)
        "gzipfory"  -> LettuceBinaryCodec(BinarySerializers.GZipFory)
        "lz4fory"   -> LettuceBinaryCodec(BinarySerializers.LZ4Fory)
        "zstdfory"  -> LettuceBinaryCodec(BinarySerializers.ZstdFory)
        else        -> LettuceBinaryCodec(BinarySerializers.LZ4Fory)
    }
