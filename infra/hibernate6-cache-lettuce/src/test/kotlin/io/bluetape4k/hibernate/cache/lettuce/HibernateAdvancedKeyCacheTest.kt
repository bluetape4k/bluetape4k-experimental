...
class HibernateAdvancedKeyCacheTest: AbstractHibernateNearCacheTest() {
    ...
    fun `composite id entity는 2nd level cache를 사용하고 Redis key는 문자열이다`() {
        ...
        val regionFactory =
            (sessionFactory as SessionFactoryImplementor)
                .serviceRegistry
    ...
    fun `natural-id cache는 hit를 만들고 Redis key는 NaturalId 문자열 표현을 사용한다`() {
        ...
        repeat(2) {
            sessionFactory.openSession().use { session ->
                session.beginTransaction()
                session
                    .byNaturalId(NaturalUser::class.java)
        ...
        redisKeys.any { key -> key.contains("##NaturalId[") } shouldBeEqualTo true
        redisKeys.any { key -> key.contains("natural@example.com") } shouldBeEqualTo true
    }
    ...
    fun `update timestamps cache도 문자열 key를 사용한다`() {
        ...
        redisKeys.any { key -> key.contains("persons") } shouldBeEqualTo true
    }
}
