# Module exposed-jdbc-spring-data
...
- Spring Boot 4 환경에서 **starter 없이도 동작** (모듈 내 auto configuration 제공)

## 최근 변경

- `@EnableExposedRepositories` 경로에서 `ExposedSpringDataAutoConfiguration`을 함께 import 하도록 보강
- `springTransactionManager`를 모듈에서 직접 구성해 `exposed_spring_boot4_starter` 의존 없이도 `@Transactional` 적용
- `DeclaredExposedQuery`의 SQL 파라미터 타입 해석을 `ColumnType.resolveColumnType` 기반으로 정리
