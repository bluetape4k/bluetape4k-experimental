...
`application.yml`에 `bluetape4k.cache.lettuce-near.*` 설정만 추가하면 별도 코드 없이
Hibernate Second Level Cache가 자동으로 활성화된다.
...
통합 테스트(`LettuceNearCacheIntegrationTest`)는 Testcontainers Redis + H2를 사용한다.
